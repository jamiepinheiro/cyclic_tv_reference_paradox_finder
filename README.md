...
| llm_reference_check.py       | Validates candidate references with an LLM (via the exe.dev LLM gateway). Replaces the old hand-maintained "too common show name" denylist: instead of dropping shows like _Lost_ or _House_ outright, each candidate match is checked in context (e.g. "we are lost" is rejected, "did you watch last night's _Lost_?" is kept). Verdicts are cached to disk. |
