...
def download_subtitles_for_season(series, season, num_episode):
    ...
    videos = set([Episode(series, series, season, e + 1)
                 for e in range(num_episode)])
    languages = {Language('eng')}
    subtitles = download_best_subtitles(videos, languages)
    for video, subs in subtitles.items():
        file_name = '%s/%s_S%s_E%s.srt' % (SUBTITLES_DIR,
                                           series, season, video.episode)
        if subs:
    ...
    with open(marker_file, 'w') as f:
        ...
        print("Found %d subtitles in %fs" %
              (subtitles_found, time.time() - start_time))
    ...
    for i in range(1000000):
        ...
        for tv_show in popular_tv_show['results']:
            ...
            for s, season in enumerate(tv_show['seasons']):
                ...
                subs_found = download_subtitles_for_season(
                    name, s + 1, season['episode_count'])
                if not subs_found:
                    break
